...
enum class CountryCode(
    ...
    companion object {
        private val numericMap: MutableMap<String, CountryCode> = java.util.HashMap()
        private const val numeric = 0

        init {
            println("Countries init")
            for (cc in values()) {
                if (cc.getNumeric() != -1) {
                    numericMap[cc.getNumeric().toString()] = cc
                }
            }
            ...
            numericMap[Integer.valueOf(104).toString()] = CountryCode.MM

            // CD and ZR have the same numeric code 180. CD should be used.
            numericMap[Integer.valueOf(180).toString()] = CountryCode.CD

            // FI and SF have the same numeric code 246. FI should be used.
            numericMap[Integer.valueOf(246).toString()] = CountryCode.FI

            // GB and UK have the same numeric code 826. GB should be used.
            numericMap[Integer.valueOf(826).toString()] = CountryCode.GB

            // TL and TP have the same numeric code 626. TL should be used.
            numericMap[Integer.valueOf(626).toString()] = CountryCode.TL

            // County code 280 is also used for DE by the German banking industry.
            numericMap[Integer.valueOf(280).toString()] = CountryCode.DE
        }

        @JsonCreator(mode = JsonCreator.Mode.DELEGATING)
        fun create(@JsonProperty str: String): CountryCode? {
